...
import br.com.cinema.cinema.service.GeneroService;
...
public class GeneroController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<Genero> updateGenero(@RequestBody Genero genero, @PathVariable int id){
		return new ResponseEntity <>(generoService.updateGenero(genero, id),HttpStatus.OK);
	}
	
	@DeleteMapping("/{id}")
	public ResponseEntity<Genero> deleteGenero(@PathVariable int id) {
		Genero genero = generoService.getGeneroById(id);
		if(null == genero)
			return new ResponseEntity <>(genero,HttpStatus.NOT_FOUND);
		else
